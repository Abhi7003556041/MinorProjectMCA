...
module.exports = {
  ...
  facultySignup: async (req, res) => {
    ...
    try {
      ...
      if (faculty.length < 10) {
        helper = "00" + faculty.length.toString();
      } else if (faculty.length < 100 && faculty.length > 9) {
        helper = "0" + faculty.length.toString();
      } else {
        helper = faculty.length.toString();
      }
      const avatar = gravatar.url(email, { s: "200", r: "pg", d: "mm" });

      var date = new Date();
      const joiningYear = date.getFullYear();
      var components = ["FCL", date.getFullYear(), departmentHelper, helper];
      ...
      const token = createToken({
        ...
        facultyMobileNumber: req.body.facultyMobileNumber,
  ...
  fetchStudents: async (req, res, next) => {
    try {
      ...
      console.log("department", department,year);

      const subjectList = await Subject.find({ department, year });
      console.log("object", subjectList);
